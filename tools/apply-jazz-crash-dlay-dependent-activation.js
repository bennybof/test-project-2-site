...
const reportJsonPath = path.join(root, "reports", "jazz-crash-dlay-dependent-activation-report.json");
const reportTxtPath = path.join(root, "reports", "jazz-crash-dlay-dependent-activation-report.txt");
...
const sourceKey = "samples/jazz_crash_metal_odd (consolidated).wav";
const targetKey = "samples/jazz_crash_metal_dlay_odd_xtra (consolidated).wav";
...
const dependentRuleResult = ensureDependentRule(
  ...
  targetKey,
  0.4,
  "dependent_jazz_crash_dlay_after_jazz_crash",
  "Saved definitions absolute final 2: jazz_crash_metal_dlay_odd_xtra can only activate if jazz crash is active. User confirmed it should roll at 40% after jazz crash activates."
);
...
const dependentOnlyResult = markDependentOnlyTarget(
  ...
  catalogKeys,
  targetKey,
  "Dependent activation target only: jazz_crash_metal_dlay_odd_xtra should only roll after jazz_crash_metal_odd activates."
);
...
const report = {
  ...
  batch: "jazz crash dlay dependent activation",
  catalogPath: "data/full-rules-catalog.json",
  note: "Adds jazz_crash_metal_dlay_odd_xtra as a dependent activation target of jazz crash at 40%, and prevents independent direct activation.",
  dependentRuleResult,
  dependentOnlyResult
};
...
const lines = [];
lines.push("TEST PROJECT 2 - JAZZ CRASH DLAY DEPENDENT ACTIVATION REPORT");
lines.push("");
lines.push(`Generated: ${report.generatedAt}`);
lines.push("Renderer change: none");
lines.push("Catalog change: jazz_crash_metal_dlay_odd_xtra dependent activation");
lines.push("");
lines.push("Rule:");
lines.push("- jazz_crash_metal_dlay_odd_xtra has 40% chance after jazz_crash_metal_odd activates.");
lines.push("- jazz_crash_metal_dlay_odd_xtra is marked dependentActivationOnly so it does not roll independently.");
lines.push("");
