...
function scheduleMidiPattern({
  ...
  function getIsolatedNumberedSequenceTestKey() {
    const rawValue =
      getRuleUrlParam("sequenceOnly") ||
      getRuleUrlParam("forceSequenceOnly") ||
      getRuleUrlParam("isolatedSequence");
  ...
  async function renderIsolatedNumberedSequenceTest({ format, sampleRate, key }) {
    const entry = getCatalogEntry(key);

    if (!entry) {
      throw new Error(`Isolated numbered sequence test entry not found: ${key}`);
    }

    const sequence = getNormalNumberedAudioSequenceForEntry(entry);

    if (!sequence.length) {
      throw new Error(`Isolated numbered sequence test has no normal sequence: ${key}`);
    }

    setStatus(`LOADING ISOLATED SEQUENCE TEST / ${entry.key} / ${sequence.length} PARTS`);
    ...
    for (const item of sequence) {
      const buffer = await fetchAndDecode(decodeContext, item.key);
      buffers.set(item.key, buffer);
      sequenceDuration += buffer.duration;
    }

    const renderDuration = Math.max(sequenceDuration + 1, 1 / sampleRate);
    const offlineContext = new OfflineAudioContext(
      2,
    ...
    const mainBpm = Number(catalog?.rulePools?.timing?.mainBpm || 56);
    const barSeconds = 60 / mainBpm * 4;

    const section = {
      id: "isolated_numbered_sequence_test",
      type: "normal",
      startSeconds: 0,
      endSeconds: renderDuration,
      barSeconds,
      tags: ["normal", "isolated_numbered_sequence_test"],
      selectedAudio: sequence.map(item => item.key),
      plannedAudio: sequence.map(item => item.key),
      forcedNumberedSequenceTestKeys: [entry.key]
    };
    ...
    const scheduledCount = scheduleNormalNumberedAudioSequence({
      offlineContext,
      destination: offlineContext.destination,
      section,
      key: entry.key,
      entry,
      random: mulberry32((currentSeed ^ 0x51E9A11C) >>> 0),
      plan,
      playbackState: createPlaybackRuleState(),
      lifecycleStates: new Map()
    });

    if (!scheduledCount) {
      throw new Error(`Isolated numbered sequence test scheduled 0 parts: ${key}`);
    }

    setStatus(`RENDERING ${format.toUpperCase()} / ISOLATED SEQUENCE TEST / PARTS ${sequence.length}`);

    const renderedBuffer = await offlineContext.startRendering();

    if (format === "wav") {
      const wavBlob = audioBufferToWavBlob(renderedBuffer);
      downloadBlob(wavBlob, `test-project-2-isolated-sequence-${currentSeed}.wav`);
    }

    if (format === "mp3") {
      await ensureLameJs();
      const mp3Blob = audioBufferToMp3Blob(renderedBuffer);
      downloadBlob(mp3Blob, `test-project-2-isolated-sequence-${currentSeed}.mp3`);
    }
  }
